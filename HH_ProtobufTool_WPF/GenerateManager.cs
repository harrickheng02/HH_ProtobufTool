...
{
    internal static class GenerateManager
    {
        /// <summary>
        /// 协议号起始范围
        /// </summary>
        public static readonly Dictionary<int, ushort> StartProtoId = new Dictionary<int, ushort>
        {
            [-1] = 0,
            [0] = 10000,
            [1] = 11000,
        ...
        public delegate void GenerationCompletedHandler(long elapsedMs);

        public delegate void GenerationProgressHandler(int current, int total, string message);

        public static async Task GenerateProtocolsAsync(GenerationProgressHandler progress = null,
            GenerationCompletedHandler completed = null)
        {
            var stopwatch = Stopwatch.StartNew();
            try
            {
                const int totalSteps = 11; // 文件处理+10个后续任务
                var currentStep = 0;
                await ParseAllAsync();
                currentStep++;
                // 执行后续生成任务
                var generationTasks = new Action[]
                {
                    () =>
                    {
                        CopyProtoToClient();
                        progress?.Invoke(++currentStep, totalSteps, "拷贝协议到客户端");
                    },
                    () =>
                    {
                        CopyPbToClient();
                        progress?.Invoke(++currentStep, totalSteps, "拷贝PB文件到客户端");
                    },
                    () =>
                    {
                        CreateCSharpProtoId();
                        progress?.Invoke(++currentStep, totalSteps, "创建C#协议ID");
                    },
                    () =>
                    {
                        CreateCSharpListener();
                        progress?.Invoke(++currentStep, totalSteps, "创建C#监听器");
                    },
                    () =>
                    {
                        CreateCSharpHandler();
                        progress?.Invoke(++currentStep, totalSteps, "创建C#处理器");
                    },
                    () =>
                    {
                        CreateLuaProtoDef();
                        progress?.Invoke(++currentStep, totalSteps, "创建Lua协议定义");
                    },
                    () =>
                    {
                        CreateLuaListener();
                        progress?.Invoke(++currentStep, totalSteps, "创建Lua监听器");
                    },
                    () =>
                    {
                        CreateLuaHandler();
                        progress?.Invoke(++currentStep, totalSteps, "创建Lua处理器");
                    },
                    () =>
                    {
                        CreateServerProtoId();
                        progress?.Invoke(++currentStep, totalSteps, "创建服务器协议ID");
                    },
                    () =>
                    {
                        CopyProtoServer();
                        progress?.Invoke(++currentStep, totalSteps, "拷贝协议到服务器");
                    }
                };

                for (int i = 0, len = generationTasks.Length; i < len; ++i)
                {
                    await Task.Run(generationTasks[i]);
                }
            }
            finally
            {
        ...
        private static void CopyProtoToClient()
        {
            ...
            for (int i = 0, len = files.Length; i < len; ++i)
            {
                ...
                if (fileInfo.Name.IndexOf("NS2GS", StringComparison.Ordinal) > -1 ||
                    fileInfo.Name.IndexOf("GS2NS", StringComparison.Ordinal) > -1)
                {
                    continue;
                }

                File.Copy(fileInfo.FullName, Path.Combine(Config.ClientOutProtoPath, fileInfo.Name), true);
            }
        }
        ...
        {
            // 把这些文件复制到目标目录
            var files = Directory.GetFiles(Path.Combine(Config.ProtocPath, "pb"));
            for (int i = 0, len = files.Length; i < len; ++i)
            {
                var fileInfo = new FileInfo(files[i]);
                if (fileInfo.Name.IndexOf("NS2GS", StringComparison.Ordinal) > -1 ||
                ...
                File.Copy(fileInfo.FullName,
                    Path.Combine(Config.ClientOutLuaPBPath, fileInfo.Name.Replace(".pb", ".bytes")),
                    true);
            }
        }
    ...
    public class Proto
    {
        ...
        public Proto()
        {
            mProtoId = new Lazy<ushort>(() => ++GenerateManager.StartProtoId[Category]);
        }
